...
## v3.0 graduate epidemiology expansion

LIPPS now contains a two-course graduate epidemiology pathway:

- **Epidemiology I — Epidemiologic Reasoning, Design and Interpretation**: expanded 12-week foundation with MSc-level deepening, eight R laboratories and graduate assessment architecture.
- **Epidemiology II — Advanced Epidemiologic Methods and Causal Inference**: new 12-week advanced sequence covering estimands, standardisation, regression, advanced case-control methods, survival, DAGs, g-methods, time-varying confounding, mediation, quantitative bias analysis, quasi-experimental methods, target-trial emulation, transportability and triangulation.

The public R code blocks are display-only, so the Quarto site does not require an R runtime merely to render the teaching pages. Synthetic teaching datasets are included under each course's `data/` folder.
